...
const config = {
    ...
    simulation: {
        ...
        noiseSpeed: 0.03, // Reduced from 0.1 for smoother evolution
        noiseScale: 1.2,
        // How strongly the particles are pushed away from the mouse
        mouseRepulsion: 0.002, // Reduced from 0.005 for a softer reaction
    ...
    useEffect(() => {
        ...
        const animate = () => {
            ...
            for (let i = 0; i < particleCount; i++) {
                ...
                // Update velocity (reduced curl force multiplier from 0.001 to 0.0003 for slower/smoother movement)
                velocities[i3] += (curlForce.x * 0.0003 + mouseForce.x * config.simulation.mouseRepulsion);
                velocities[i3 + 1] += (curlForce.y * 0.0003 + mouseForce.y * config.simulation.mouseRepulsion);
                velocities[i3 + 2] += (curlForce.z * 0.0003 + mouseForce.z * config.simulation.mouseRepulsion);
